...
export default function CourierPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError('');
    try {
      setData(await apiRequest<Dashboard>('/api/staff/dashboard'));
    } catch (loadError) {
      setData(null);
  ...
  const shipments = useMemo(
    () =>
      [...(data?.shipments ?? [])].sort(
        (first, second) =>
          (first.route_order ?? 9999) - (second.route_order ?? 9999) ||
          first.delivery_window.localeCompare(second.delivery_window),
      ),
    [data?.shipments],
  );
  const active = shipments.filter(
    (shipment) => !completedStatuses.includes(shipment.status),
  );
  const completed = shipments.filter((shipment) =>
    completedStatuses.includes(shipment.status),
  );
  const codTotal = active.reduce(
    (sum, shipment) =>
      sum + (shipment.cod_status === 'pending' ? shipment.cod_amount_all : 0),
    0,
  );
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#071b33] text-white">
        <div className="text-center">
          <RefreshCw className="mx-auto size-9 animate-spin text-orange-400" />
          <p className="mt-4 font-bold">Po ngarkojmë itinerarin...</p>
        </div>
      </main>
    );
  if (!data) return <CourierLogin onLogin={load} initialError={error} />;
  if (data.staff.role !== 'courier')
    return (
      <main className="grid min-h-screen place-items-center bg-[#f4f6f9] p-5">
        <div className="max-w-md rounded-3xl bg-white p-8 text-center shadow-xl">
          <CircleUserRound className="mx-auto size-12 text-orange-500" />
          <h1 className="mt-4 text-2xl font-black">Panel vetëm për korrierë</h1>
          <p className="mt-3 text-slate-500">
            Llogaria juaj përdor panelin e plotë të stafit.
          </p>
          <Link
            href="/staff"
            className="mt-6 inline-flex rounded-xl bg-[#071b33] px-5 py-3 font-black text-white"
          >
            Hap panelin e stafit
          </Link>
        </div>
      </main>
    );
    ...
    <main className="min-h-screen bg-[#eef2f6] pb-28 text-[#10233d]">
      <header className="sticky top-0 z-30 bg-[#071b33] px-4 pb-5 pt-[max(1rem,env(safe-area-inset-top))] text-white shadow-xl">
        <div className="mx-auto max-w-xl">
          ...
          <div className="mt-5">
            <p className="text-sm text-slate-400">Mirë se erdhe,</p>
            <h1 className="text-2xl font-black">{data.staff.fullName}</h1>
          </div>
          <div className="mt-5 grid grid-cols-3 gap-2">
            <Metric icon={Route} value={active.length} label="Ndalesa" />
            <Metric
              icon={PackageCheck}
              value={
                completed.filter((item) => item.status === 'U dorëzua').length
              }
              label="Mbaruar"
            />
            <Metric icon={Banknote} value={codTotal} label="COD Lekë" />
          </div>
        </div>
      </header>
      ...
      <section className="mx-auto max-w-xl px-4 py-5">
        ...
        <div className="mb-5 grid grid-cols-2 rounded-2xl bg-white p-1 shadow-sm">
          <button
            ...
            className={`rounded-xl py-3 text-sm font-black ${view === 'active' ? 'bg-orange-500 text-white' : 'text-slate-500'}`}
          >
            Itinerari sot ({active.length})
          </button>
          <button
            onClick={() => setView('completed')}
            className={`rounded-xl py-3 text-sm font-black ${view === 'completed' ? 'bg-[#071b33] text-white' : 'text-slate-500'}`}
          >
            Të përfunduara ({completed.length})
          </button>
        </div>
        <div className="space-y-4">
          {(view === 'active' ? active : completed).map((shipment) => (
            <StopCard
              key={shipment.id}
              shipment={shipment}
              onUpdated={load}
              onProof={() => setProofShipment(shipment)}
            />
          ))}
          {!(view === 'active' ? active : completed).length && (
            <div className="rounded-3xl border-2 border-dashed border-slate-300 bg-white p-10 text-center">
              ...
              <h2 className="mt-4 text-xl font-black">Asnjë ndalesë këtu</h2>
              <p className="mt-2 text-sm text-slate-500">
                Itinerari përditësohet nga dispeçeri.
              </p>
            </div>
          )}
        </div>
      ...
      <nav className="fixed inset-x-0 bottom-0 z-30 border-t border-slate-200 bg-white/95 pb-[max(.75rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur">
        <div className="mx-auto grid max-w-xl grid-cols-3 px-5 text-center text-xs font-black">
          <button
            onClick={() => setView('active')}
            className="grid justify-items-center gap-1 text-orange-600"
          >
          ...
          <button
            onClick={load}
            className="grid justify-items-center gap-1 text-slate-500"
          >
...
function StopCard({
  ...
  const destination = encodeURIComponent(
    `${address}, ${shipment.delivery_city}, Albania`,
  );
  const whatsappPhone = shipment.recipient_phone
    .replace(/\D/g, '')
    .replace(/^0/, '355');
  const nextStatus =
    shipment.status === 'Porosia u regjistrua' ||
    shipment.status === 'Në pritje të marrjes'
      ? 'U mor nga korrieri'
      : shipment.status === 'U mor nga korrieri'
        ? 'Në transport'
        : 'Në shpërndarje';
    ...
    try {
      await apiRequest(`/api/staff/shipments/${shipment.id}`, {
        ...
        body: JSON.stringify({
          ...
          details: `Dërgesa u përditësua nga korrieri: ${nextStatus}.`,
          latitude: null,
          longitude: null,
    ...
    <article className="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
      <div className="border-b border-slate-100 p-5">
        <div className="flex items-start justify-between gap-3">
          <div className="flex items-center gap-3">
            <span className="grid size-11 shrink-0 place-items-center rounded-2xl bg-[#071b33] text-lg font-black text-white">
            ...
            <div>
              ...
              <p className="mt-1 text-xs font-bold text-slate-500">
                {shipment.service.toUpperCase()} · {shipment.weight_kg} kg
              </p>
            </div>
          </div>
        ...
        {shipment.cod_amount_all > 0 && (
          <p className="mt-4 rounded-xl bg-amber-50 p-3 text-sm font-black text-amber-800">
            Mblidh {shipment.cod_amount_all} Lekë në dorëzim
          </p>
        )}
      </div>
      <div className="grid grid-cols-3 gap-2 p-4">
        <a
          ...
          className="grid place-items-center gap-1 rounded-2xl bg-slate-100 px-2 py-3 text-xs font-black"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="grid place-items-center gap-1 rounded-2xl bg-emerald-50 px-2 py-3 text-xs font-black text-emerald-700"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="grid place-items-center gap-1 rounded-2xl bg-blue-50 px-2 py-3 text-xs font-black text-blue-700"
        >
      ...
      {!completedStatuses.includes(shipment.status) && (
        <div className="grid gap-2 border-t border-slate-100 p-4">
          <button
            onClick={advance}
            disabled={saving || shipment.status === 'Në shpërndarje'}
            className="flex items-center justify-between rounded-2xl bg-[#071b33] px-5 py-4 text-sm font-black text-white disabled:opacity-40"
          >
            <span>
              {saving
                ? 'Duke ruajtur...'
                : shipment.status === 'Në shpërndarje'
                  ? 'Gati për dorëzim'
                  : `Kalo në: ${nextStatus}`}
            </span>
          ...
          <button
            onClick={onProof}
            className="flex items-center justify-center gap-2 rounded-2xl bg-orange-500 px-5 py-4 text-sm font-black text-white"
          >
...
function DeliveryProof({
  ...
  function locate() {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => setError('Lejoni GPS-in në shfletues dhe provoni përsëri.'),
  ...
  async function submit(event: SyntheticEvent<HTMLFormElement>) {
    ...
    if (!signed || !canvasRef.current) {
      setError('Marrësi duhet të firmosë.');
      return;
    }
    setSaving(true);
    setError('');
    ...
    try {
      await apiRequest(`/api/staff/shipments/${shipment.id}/proof`, {
        method: 'POST',
        body: form,
      });
      await onSaved();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end bg-[#071b33]/75 backdrop-blur-sm">
      <dialog
        open
        className="m-0 max-h-[96vh] w-full overflow-y-auto rounded-t-[2rem] bg-white p-5 text-[#10233d]"
      >
        <div className="mx-auto max-w-xl">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            <div>
              <div className="mb-2 flex items-center justify-between">
                <p className="text-sm font-black">Firma e marrësit</p>
                <button
                  type="button"
                  onClick={clear}
            ...
            <label className="block text-sm font-black">
              Foto e dorëzimit (JPG/PNG, deri në 5 MB dhe 2 MP)
              <input
                type="file"
                accept="image/jpeg,image/png"
                capture="environment"
                onChange={(event) => setPhoto(event.target.files?.[0] ?? null)}
                className="mt-2 block w-full rounded-xl bg-slate-100 p-3 text-sm"
              />
            </label>
            <button
              type="button"
              onClick={locate}
              className="flex w-full items-center justify-between rounded-2xl bg-blue-50 p-4 text-left"
            >
              <span>
                <strong className="block text-sm">Pozicioni GPS</strong>
                <span className="text-xs text-blue-700">
                  {latitude && longitude
                    ? `${latitude.toFixed(5)}, ${longitude.toFixed(5)}`
                    : 'Prek për të regjistruar vendndodhjen'}
                </span>
              </span>
            ...
            {shipment.cod_amount_all > 0 && (
              <p className="rounded-2xl bg-amber-50 p-4 text-sm font-black text-amber-900">
                Konfirmoni mbledhjen e {shipment.cod_amount_all} Lekë COD.
              </p>
            )}
            {error && (
  ...
  return (
    <main className="grid min-h-screen place-items-center bg-[#071b33] px-5 py-10">
      ...
      >
        ...
        />
        <p className="mt-8 text-center text-xs font-black uppercase tracking-[0.18em] text-orange-500">
          Paneli i korrierit
        </p>
        <h1 className="mt-2 text-center text-3xl font-black text-[#10233d]">
          Itinerari në xhep
        </h1>
